...
if event_data[? "event_type"] == "sprite event" // or you can check "sprite event"
{
    switch (event_data[? "message"])
    {
		...
		case "robot_limb1":
			audio_play_sound(sfx_robotlimbmove, 10, false,1,0,.6+random_range(.9, 1.1) + abs(hsp)/50);
		break;
		case "robot_limb2":
			audio_play_sound(sfx_robotlimbmove, 10, false,1,0,.3+random_range(.9, 1.1) + abs(hsp)/50);
		break;
    }
}
}