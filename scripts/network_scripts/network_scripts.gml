...
function send_spawn_packet(_socket, _player_id) {
    ...
    network_send_raw(_socket, buff, buffer_tell(buff));
    buffer_delete(buff);
}

function send_player_appearance(_socket, _slot, _char, _suit, _hair, _id) {
    var payload = buffer_create(32, buffer_grow, 1);

    buffer_write(payload, buffer_string, _char);
    buffer_write(payload, buffer_u32, _suit);
    buffer_write(payload, buffer_string, _hair);
    buffer_write(payload, buffer_u8, _id);

    send_framed_packet(_socket, _slot, 2, payload);
...
/// send_framed_packet(socket, sender_slot, packet_type, payload_buffer)
function send_framed_packet(_sock, _slot, _type, _payload)
{
    var payload_size = buffer_tell(_payload);

    var buff = buffer_create(2 + 1 + 1 + payload_size, buffer_fixed, 1);

    // length = slot + type + payload
    buffer_write(buff, buffer_u16, 1 + 1 + payload_size);

    buffer_write(buff, buffer_u8, _slot);
    buffer_write(buff, buffer_u8, _type);

    buffer_copy(buff, buffer_tell(buff), _payload, 0, payload_size);

    var did_it_work = network_send_raw(_sock, buff, buffer_tell(buff));

	if (did_it_work < 0){
	audio_play_sound(uidecline, 0, false);
	}
    buffer_delete(buff);
}
