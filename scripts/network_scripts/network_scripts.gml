...
function scr_client_start_punch(){
	...
	for (var i = 0; i < 20; i++) {
		...
		network_send_udp(client_socket, global.target_ip, UDP_SERVER_PORT, buff, buffer_tell(buff));
	}

	buffer_delete(buff);

	show_debug_message("NAT PUNCHING TOWARDS: " + string(global.target_ip) + ":" + string(global.target_port) + " (and port " + string(UDP_SERVER_PORT) + ")");
}
