...
if (string_pos("/resolve/", url) > 0) {
	...
	var connected_to_relay = network_connect_raw(global.tcp_sock, global.relay_ip, global.relay_tcp);
	...
	if (buffer_exists(accum)) buffer_delete(accum);
	accum = buffer_create(1, buffer_grow, 1);

	
	
	//if (connected_to_relay >= 0) {
	//var payload = buffer_create(1, buffer_grow, 1); 
	// no payload for fake connect
	//audio_play_sound(uinotif, 0, false);
	//send_framed_packet(global.tcp_sock, 0, 1, payload);
